...
@section('content')
    <h1 class="section-title">Modifier la vente #{{ $vente->id }}</h1>

    <div class="card p-4" style="max-width: 800px;">
        <form method="POST" action="{{ route('ventes.update', $vente) }}" id="formVente">
            @csrf
            @method('PUT')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Calcul initial au chargement pour la vente existante
            calculerMontant();
        });
    </script>
@endsection
